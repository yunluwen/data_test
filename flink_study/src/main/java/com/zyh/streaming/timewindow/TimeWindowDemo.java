package com.zyh.streaming.timewindow;
...
/**
 * 测试:
 * eventTime的window触发机制
 */
public class TimeWindowDemo {

    public static void main(String[] args) throws Exception {
        final long delay = 5100L;
        int windowSize = 15;
        ...
        DataStream<Tuple3<String, String, Long>> dataStream =
                env.addSource(new TimeWindowDemo.DataSource()).name("Demo Source");

        // 设置水位线
        DataStream<Tuple3<String, String, Long>> watermark =
                dataStream.assignTimestampsAndWatermarks(
                        new AssignerWithPeriodicWatermarks<Tuple3<String, String, Long>>() {
            private final long maxOutOfOrderness = delay;   //允许的最大乱序时间
            private long currentMaxTimestamp = 0L;          //当前最大时间戳
        ...
        // 窗口函数进行处理
        DataStream<Tuple3<String, String, Long>> resStream =
                watermark.keyBy(0)
                .timeWindow(Time.seconds(windowSize)).reduce(
                new ReduceFunction<Tuple3<String, String, Long>>() {
                    @Override
                    public Tuple3<String, String, Long>
                    reduce(Tuple3<String, String, Long> value1,
...
/**
 * 如果当前数据的 EventTime 在 WaterMark 之上，也就是 EventTime> WaterMark。
 *      因为我们知道数据所属窗口的 WindowEndTime，一定是大于 EventTime 的。
 *      这时我们有 WindowEndTime > EventTime > WaterMark。所以这种情况下数据是一定不会丢失的。
 *
 * 如果当前数据的 EventTime 在 WaterMark 之下，也就是 WaterMark > EventTime。这时候要分两种情况：
 *   2.1 如果该数据所属窗口的 WindowEndTime > WaterMark，则表示窗口还没被触发，
 *       即 WindowEndTime > WaterMark > EventTime，这种情况数据也是不会丢失的。
 *
 *   2.1 如果该数据所属窗口的 WaterMark > WindowEndTime，则表示窗口已经无法被触发，
 *       即 WaterMark > WindowEndTime > EventTime，这种情况数据也就丢失了。
 */