...
object UserActiveDegreeAnalyze {
  ...
  def main(args: Array[String]) {
    // 如果是按照课程之前的模块，或者整套交互式分析系统的架构，应该先从mysql中提取用户指定的参数（java web系统提供界面供用户选择，
    // 然后java web系统将参数写入mysql中）
    ...
    // 获取两份数据集
    val userBaseInfo = spark.read.json("data\\user_base_info.json")
    val userActionLog = spark.read.json("data\\user_action_log.json")
